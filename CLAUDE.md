...
**"Wake + Script" (`AgentConfig.wakeWithScriptEnabled`)** is a third flag of the same shape (`ManualScriptFlag`, `util/wol/manualScriptFlag.ts`, `POST /:id/manual-script-flag/consume`) rather than folding a "force" option into the wake/WOL-detection flow itself — same "dedicated entity+function pair per concern" reasoning. `POST /:id/wake`'s body accepts an optional `forceManualBootScript: true` (`WakeRequestSchema`), which arms this flag alongside the normal wake flag in the same request — it does not change what gets sent over the wire (still just the one magic packet) or how `wol-flag/consume` behaves. On the agent side, `service.ts`'s `runBootTimeScripts` checks `manual-script-flag/consume` **unconditionally**, before and independent of the `wolAware` branch — the whole point is forcing `manualBootScript` to run on a boot that *is* WOL-triggered (which would otherwise run `wolScript` instead, or nothing), so it can't be gated on the same WOL detection it's meant to override. A `ranManualBootScript` guard prevents it from also running a second time via the `wolAware && !woken` branch if both happen to apply.
...
That covers a shutdown *we* requested — it does nothing for a regular OS shutdown/reboot/service restart the user triggers directly on the machine, since that never touches `shutdown-flag/consume` at all. `POST /api/v1/targets/:id/offline` closes that gap from the other side: `service.ts` registers a SIGINT/SIGTERM/SIGBREAK handler (`util/gracefulShutdown.ts`'s `createShutdownHandler`, built with injected deps so it's unit-testable without a real process/network) that fires this as a best-effort beacon the moment the OS/NSSM asks the process to stop, racing it against a short timeout so a slow request can't delay exit and risk an escalation to a hard `TerminateProcess`. It's still only best-effort — a hard power-loss or crash delivers no signal at all, and `AGENT_STALE_THRESHOLD_SECONDS` remains the fallback for that case.

### Windows agent

`src/agent/` is a **separate compiled binary**, not part of the server — built via `bun build --compile --minify --outfile=... src/agent/main.ts` (native on the target OS; cross-compilable from any OS via `--target=bun-windows-x64`, verified). It's a single Windows Service (NSSM-wrapped, see `installer/`), started via `main.ts` → `service.ts`'s `runService`, no CLI mode flag involved.

**This used to be two separate execution contexts — it deliberately isn't anymore.** The original design split things into a SYSTEM service (heartbeat + shutdown-flag polling, `service.ts`) and a *separate* Scheduled Task firing at any user's logon (`bootHooks.ts`, since removed — WOL-boot detection + script execution), reasoning that a service running in Windows' Session 0 is isolated from the interactive desktop and couldn't reliably do anything a local script might need. That turned out to be solving a problem that didn't exist for how this is actually used: script execution here is local automation (run a `.ps1`/`.bat`/`.exe`, talk to hardware over USB/serial) with no genuine dependency on an interactive session, and worse, gating it on a logon having already happened is actively backwards for a script meant to bring up a display (e.g. a CEC wake command) — nobody can see a login screen that isn't lit up yet. `runService` now runs the boot-time script/WOL-detection check once at startup (`runBootTimeScripts`, called before entering the perpetual heartbeat loop), then proceeds exactly as before. If a genuine future need for real desktop-session access ever shows up, that's the point to reconsider splitting again — not preemptively.

**`withinSeconds` for the boot-time `wol-flag/consume` check is computed from `os.uptime()`**, not a fixed constant (`src/agent/util/computeWolWithinSeconds.ts`, called from `service.ts`'s `runBootTimeScripts`): `Math.ceil(os.uptime()) + BOOT_BUFFER_SECONDS`. Now that this runs once at service startup — which happens automatically at boot, before any login — there's no separate boot→login gap to account for the way there was when this ran from a logon-triggered task; `os.uptime()` at that point is just however long this specific boot actually took. Deriving the window from the agent's own uptime (a duration, not an absolute timestamp) still avoids any cross-machine clock-sync dependency. `FlagConsumeSchema`'s `withinSeconds` cap stays raised at `14400` (from `3600`) as a defensive ceiling, even though a pathologically slow boot is now the only realistic thing that would approach it.

**The installer no longer writes `config.json` at all** — the agent resolves its own bootstrap values (`src/agent/config.ts`'s `resolveAgentConfig`, called once from `main.ts` before `runService` starts):
...
Component tests live in `tests/client/` (mirroring `src/client/`) and use `@testing-library/react` + `jsdom`. Opt into jsdom per file via `// @vitest-environment jsdom` as the first line of the file — the project's default Vitest environment is `node` (needed for server/agent tests); `environmentMatchGlobs`, the old way to scope this by directory instead, was removed in Vitest v4, so the per-file comment is the only option now. `@testing-library/jest-dom/vitest` is registered globally in `vitest.config.ts`'s `setupFiles` (safe for non-DOM tests too — it only defines matcher functions, never touches `document` at import time), so `toBeInTheDocument()` etc. work without per-file setup. `@testing-library/react`'s auto-cleanup-after-each isn't wired in globally either (this project never uses Vitest's `globals: true`), so every client test file needs its own `afterEach(() => cleanup())`.

For a **component** that consumes `TargetsContext`/`useNotification` (e.g. `TargetCard`), mock those hooks directly rather than wrapping it in the real providers — you're testing the component's own logic, not the context's. For the **context/provider itself** (`TargetsContext.tsx`), do the opposite: render the real `TargetsProvider` via `renderHook(() => useTargets(), { wrapper })` and mock only `~/client/api/targetsApi` beneath it — see `tests/client/components/targets/TargetsContext.test.tsx` for the pattern, including how to assert the `visibilitychange`/`focus` refetch listeners (dispatch the event on `document`/`window` inside `act()`, then `waitFor` the second `listTargets` call).
...
- `service.ts`/`main.ts` are orchestration only and aren't unit tested, matching how the server's own `main.ts` isn't tested either — verify these by actually running the compiled/interpreted agent against a local dev backend (`bun run src/agent/main.ts --config=<path>`) instead.
...
Every route in this app, including the new `/shutdown`, `/agent-config`, and `/status` routes, is unauthenticated — the LAN itself is the trust boundary, matching `/wake`'s existing precedent, a deliberate decision (not an oversight) made when this feature was added. The one place this needed a specific design answer: `AgentConfig.defaultScript`/`wolScript`/`manualBootScript` let the (unauthenticated) web UI point the agent at a local script to run. The server never sends script *content* — only ever a path/reference the agent runs on its own machine via `runScriptIfConfigured()`. So the actual exposure is bounded to "redirect execution to something that already exists on this specific machine," never "inject new code" — accepted as consistent with the no-auth decision, not something to silently harden with an auth layer without discussing the tradeoff first.

`service.ts`'s `runBootTimeScripts` three script fields cover three mutually exclusive boot outcomes, not two: `defaultScript` runs unconditionally (every boot, regardless of cause); when `wolAware` is on, exactly one of `wolScript` (boot was WOL-triggered) or `manualBootScript` (checked and confirmed it wasn't) runs on top of that — never both, and neither at all if `wolAware` is off, since then the WOL check itself never runs and there's nothing to confidently branch on. `manualBootScript` was added specifically because `defaultScript` firing on *every* boot doesn't give you a way to run something only when a boot was **not** WOL-triggered (e.g. skipping a CEC/display wake step that only makes sense after a remote wake) — before this, that condition had no dedicated hook at all.

`runScriptIfConfigured()` runs `scriptCommandFor(scriptPath)` (`util/scriptCommand.ts`), not the raw path — Windows' `CreateProcess` (what `Bun.spawn` uses) doesn't consult file-type associations the way Explorer/`cmd.exe` do, so a `.ps1`/`.bat`/`.cmd` reference isn't directly spawnable on its own; `scriptCommandFor` wraps `.ps1` as `powershell.exe -NoProfile -ExecutionPolicy Bypass -File <path>` and `.bat`/`.cmd` as `cmd.exe /c <path>`, spawning anything else (a `.exe`) as-is. `-ExecutionPolicy Bypass` is deliberate, not a security shortcut: PowerShell's default script-blocking exists to guard against untrusted/remote scripts, and that threat model doesn't apply to a local, admin-configured path under the no-auth model above.

A **shutdown** script (the boot-side counterpart to `defaultScript`) was considered and deliberately not built into the agent: it's only reliably runnable on the app-triggered shutdown path (before the agent calls `shutdown /s`, with no OS deadline pressure), never on a regular OS-triggered shutdown/reboot — that path only gets the same short, unextendable grace window as the `/offline` beacon (see above), which real shutdown automation (e.g. a CEC/USB-adapter command needing a multi-second adapter handshake) can't reliably fit into. Windows' own Group Policy shutdown scripts (which the OS explicitly waits on before powering off) are the correct tool for that case instead.
