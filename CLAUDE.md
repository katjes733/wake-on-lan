...
`bootHooks.ts`'s three script fields cover three mutually exclusive boot outcomes, not two: `defaultScript` runs unconditionally (every logon, regardless of cause); when `wolAware` is on, exactly one of `wolScript` (boot was WOL-triggered) or `manualBootScript` (checked and confirmed it wasn't) runs on top of that — never both, and neither at all if `wolAware` is off, since then the WOL check itself never runs and there's nothing to confidently branch on. `manualBootScript` was added specifically because `defaultScript` firing on *every* boot doesn't give you a way to run something only when a boot was **not** WOL-triggered (e.g. skipping a CEC/display wake step that only makes sense after a remote wake) — before this, that condition had no dedicated hook at all.
