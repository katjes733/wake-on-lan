...
/**
 * Upserts the target's single wake_flags row: fresh triggered_at, consumed_at
 * reset to null. Uses a raw INSERT ... ON CONFLICT rather than TypeORM's
 * .upsert() for full control over exactly which columns get overwritten, and
 * because the unique index on target_id makes this a single atomic
 * round-trip — safe even if "Wake" is double-clicked.
 */
export async function armWakeFlag(
  targetId: string,
): Promise<{ triggeredAt: Date }> {
  const ds = await AppDataSource.getInstance();
  const [row] = await ds.query(
    `INSERT INTO ${qualifiedTable("wake_flags")}
...
/**
 * Atomically checks-and-consumes the target's wake flag: if there's an
 ...
 */
export async function consumeWakeFlag(
  targetId: string,
  withinSeconds: number,
): Promise<{ woken: true; triggeredAt: Date } | { woken: false }> {
  const ds = await AppDataSource.getInstance();
  ...
  const [rows] = await ds.query(
    `UPDATE ${qualifiedTable("wake_flags")}
  ...
  if (rows.length === 0) return { woken: false };
  return { woken: true, triggeredAt: rows[0].triggered_at };
}
